...
interface UniversalLawsuitSystemProps {
...
export function UniversalLawsuitSystem({ 
  isGameActive, 
  isGameWon, 
  lawsuitsEnabled, 
  money, 
  ...
  // Return lawsuit status display even when not active (for universal display)
  const getLawsuitStatus = () => {
    if (!lawsuitsEnabled) return "Lawsuits Disabled";
    if (lawsuit.isActive) return `Active: ${lawsuit.amount.toLocaleString()} points demanded`;
    if (showFinalLawsuitScreen) return "Final Lawsuit Dismissal";
    return "No Active Lawsuits";
  };

  const getTimeToNextLawsuit = () => {
    if (!lawsuitsEnabled || lawsuit.isActive || showFinalLawsuitScreen) return null;
    
    const timeElapsed = Date.now() - lawsuit.lastLawsuitTime;
    const timeRemaining = LAWSUIT_TRIGGER_INTERVAL - timeElapsed;
    
    if (timeRemaining <= 0) return "Imminent";
    
    const seconds = Math.ceil(timeRemaining / 1000);
    return `${seconds}s`;
  };

  if (!lawsuitsEnabled && !lawsuit.isActive && !showFinalLawsuitScreen) {
    return null;
  }

  return (
    <>
      {/* Universal Lawsuit Status Display - Always visible when lawsuits are enabled */}
      {lawsuitsEnabled && !lawsuit.isActive && !showFinalLawsuitScreen && (
        <div className="w-full mb-2">
          <div className="p-2 rounded-lg border border-orange-300 bg-orange-50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-lg">⚖️</span>
                <span className="text-sm font-boogaloo text-orange-700">
                  Lawsuit System Active
                </span>
              </div>
              <div className="text-xs font-boogaloo text-orange-600">
                Status: {getLawsuitStatus()}
                {getTimeToNextLawsuit() && (
                  <span className="ml-2">Next: {getTimeToNextLawsuit()}</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center z-50">
          <div className="bg-gradient-to-r from-red-50 to-orange-50 border-4 border-red-600 rounded-xl shadow-2xl max-w-md mx-4 p-4">
            <div className="text-center">
              <h2 className="text-xl font-boogaloo text-red-700 mb-1">{getLawsuitMessage()}</h2>
              <p className="text-base font-boogaloo text-red-600 mb-2">{getLawsuitDescription()}</p>
              
              <p className="text-lg font-boogaloo text-red-800 mb-2">
                Settlement Demand: {lawsuit.amount.toLocaleString()} points
              </p>
              {lawsuit.amount > money && money >= 0 && (
                <p className="text-xs font-boogaloo text-red-600 mb-4">
              ...
              <div className="flex gap-3 justify-center">
                <Button 
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 font-boogaloo"
                >
                  SETTLE
                </Button>
                <Button 
                  onClick={handleLawsuitRefuse}
                  className="bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2 font-boogaloo"
                >
                  REFUSE TO PAY
                </Button>
              </div>
      ...
      {showFinalLawsuitScreen && !isGameWon && (
        <div className="absolute inset-0 flex items-center justify-center z-50">
          <div className="bg-gradient-to-r from-orange-50 to-yellow-50 border-4 border-orange-600 rounded-xl shadow-2xl max-w-md mx-4 p-6">
            <div className="text-center">
              <h2 className="text-xl font-boogaloo text-orange-700 mb-3">
                The great and all powerful orange one says "What Lawsuit?"
              </h2>
              <p className="text-base font-boogaloo text-orange-600 mb-4">
              ...
              <div className="flex justify-center">
                <Button 
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white text-sm px-6 py-2 font-boogaloo"
                >
                  OKAY
                </Button>
              </div>
